...
class PathSegment:
    def __init__(self, nodes: List[Node], main_index: int, secondary_index: int = 0):
        ...
        self.geometry_data = None  # Data for 3D visualization

    def adjust_start_and_endpoints(self, node_size, previous_end_point=None, next_start_point=None):
        if self.path_curve_model == 'polyline' and (self.curve_type == 'straight' or self.curve_type is None):
            if len(self.nodes) >= 2:
...
class PathArchitect:
    def __init__(self, nodes: List[Node]):
        ...
        self.main_index_counter = 0  # Main index counter
        ...
        # Initialize the segment index counter
        self.segment_index_counter = 0
    ...
    def create_start_ramp(self):
        ...
        if start_nodes:
            # Use a main_index before the first segment
            segment = PathSegment(
                start_nodes,
                main_index=-1,  # Ensures it appears first when sorted
                secondary_index=0
            )
    ...
    def create_finish_box(self):
        ...
        if end_nodes:
            # Use a main_index after the last segment
            segment = PathSegment(
                end_nodes,
                main_index=self.main_index_counter + 1,
                secondary_index=0
            )
