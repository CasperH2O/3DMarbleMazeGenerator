...
class PathSegment:
    ...
    def _check_and_handle_bridging_segment(self, previous_end_point, next_start_point) -> bool:
        ...
        # Conversion for any Node to b3d.Vector:
        def as_vector(n: Node) -> b3d.Vector:
        ...
        if len(self.nodes) == 1:
            ...
            # If none of these bridging scenarios match, return False to continue adjustments.
            return False
    ...
    def _adjust_multi_node_segment(self, node_size,
                                   ...
                                   next_curve_type):
        """
        Handles the standard logic for segments that have 2 or more nodes 
        (adjusting the start node, puzzle_end node, etc.).
        """
        # Adjust, unless it's the start node
        if not self.nodes[0].puzzle_start:
            ...
            # If there's a previous curve, we use full node_size, else half
            ...
            # If the first node is already puzzle_start, just mark it
        ...
        if self.nodes[-1].puzzle_end:
            ...
            # If there's a next curve, we use full node_size, else half
    ...
    def _adjust_single_node_segment(self, node_size,
                                    ...
                                    next_curve_type):
        """
        Handles single-node segments that are not puzzle_end bridging segments.
        This might be a mounting node or other single-node scenario.
        """
        ...
        # If this single node is puzzle_end, move it by half the node size
        if self.nodes[0].puzzle_end:
            ...
            # If it's neither puzzle_start nor puzzle_end, 
            # we might want to create a start and end node around it.
            if not self.nodes[0].puzzle_start and not self.nodes[0].puzzle_end:
                ...
                # If it's puzzle_start or puzzle_end, just mark it accordingly.
